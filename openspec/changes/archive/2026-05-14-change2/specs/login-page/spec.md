...
The `LoginPage` SHALL call the `login()` function from `useAuth()` when the form is submitted.

#### Scenario: Form submission triggers login
- **WHEN** the user fills in credentials and clicks "Login"
- **THEN** the `login()` function is called with the entered username and password
